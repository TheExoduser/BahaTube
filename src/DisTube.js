...
class DisTube extends EventEmitter {
	...
	constructor(client, otp = {}) {
		...
		if (typeof otp.customFilters === "object") {
			Object.assign(this.filters, otp.customFilters);
		}
	...
	async _resolveSong(message, song, type = "yt") {
		...
		if (song instanceof SearchResult) {
			return new Song(await ytdl.getInfo(song.url, {requestOptions: this.requestOptions}), message.author, true);
		}
		...
		if (ytdl.validateURL(song)) {
			return new Song(await ytdl.getInfo(song, {requestOptions: this.requestOptions}), message.author, true);
		}
		if (isURL(song)) {
			let info = await youtube_dl.getInfo(song, youtube_dlOptions).catch(e => {
				throw new Error(`[youtube-dl] ${e.stderr}`)
			});
		...
		return this._resolveSong(message, await this._searchSong(message, song, true, 1));
	}
	...
	async _handleSong(message, song, skip = false) {
		...
		if (Array.isArray(song)) {
			this._handlePlaylist(message, song, skip);
	 ...
	 * @returns {string|null}
	 */
	getPlayTime(message, queue) {
		if (!queue) {
			return null;
		}
		return formatDuration(queue.currentTime);
	}
	 ...
	 */
	async search(string, retried = false, limit = 15) {
		try {
	 ...
	 */
	async _searchSong(message, name, emit = true, limit = 15) {
		let results = await this.search(name, limit);
	...
	async addRelatedVideo(message) {
		...
		if (!song.youtube) {
			// Search related
			let search = await this._resolveSong(message, await this._searchSong(message, queue.songs[0].name, false, 1));
			search.type = "yt";
		...
		if (!related) {
			related = (await ytdl.getBasicInfo(song.url, {requestOptions: this.requestOptions})).related_videos;
		}
		if (related && related[0]) {
			this._addToQueue(message, new Song(await ytdl.getInfo(related[0].id), this.client.user, true));
		} else {
			this.emit("noRelated", message);
		}
		return queue;
	...
	setFilter(message, filter) {
		...
		if (!Object.prototype.hasOwnProperty.call(this.filters, filter)) {
			throw new TypeError(`${filter} is not a Filter (https://DisTube.js.org/global.html#Filter).`);
		}
	...
	async _handleSongFinish(message, queue) {
		...
		if (queue.songs.length <= 1 && (queue.skipped || !queue.repeatMode)) {
			if (queue.autoplay) {
				await this.addRelatedVideo(message);
			}
	...
	_handlePlayingError(message, queue) {
		...
		if (queue.songs.length > 0) {
			this._playSong(message).then(() => this.emit("playSong", message, queue, queue.songs[0]));
		} else {
			try {
				this.stop(message)
			} catch {
